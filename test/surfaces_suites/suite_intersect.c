...
void test_surfaces_poly_intersect_o2()
{
    ...
    CU_ASSERT_DOUBLE_EQUAL((*(coords + 0)).x, -0.767592, granularity);
    CU_ASSERT_DOUBLE_EQUAL((*(coords + 0)).y, 2.178395, granularity);
    CU_ASSERT_DOUBLE_EQUAL((*(coords + 1)).x, 0.434259, granularity);
    CU_ASSERT_DOUBLE_EQUAL((*(coords + 1)).y, 1.377161, granularity);
    free(coords);

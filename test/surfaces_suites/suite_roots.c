...
void test_surfaces_roots_factory_o2a()
{
    ...
    poly_check(o2);
    polynomial_setfactor(o2, 3.0f, p);
    polynomial_setfactor(o1, 1.0f, p);
    polynomial_setfactor(o0, -1.0f, p);
    ivl = (interval_t){.l = -1, .h = 1};
    nbr = roots_factory(p, ivl, r);
    CU_ASSERT_EQUAL(nbr, o2);
    CU_ASSERT_DOUBLE_EQUAL(*(r + 0), -0.767592, granularity);
    CU_ASSERT_DOUBLE_EQUAL(*(r + 1), 0.434259, granularity);
    reset_test_roots();
}
